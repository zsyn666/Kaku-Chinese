...
- `.agents/skills/` - the canonical and tracked home for project skills: `release`, `maintainer-sweep`, `product-docs`, and `bugs`. `.claude/` is gitignored except for the force-added `.claude/rules/macos.md`; its skill entries may be relative symlinks or ignored private overrides. Do not cite, copy, or edit an ignored `.claude/skills/` directory as project truth. Put durable skill changes in `.agents/skills/` first, then update a symlink when that entry is actually a link.
